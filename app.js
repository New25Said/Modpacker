...
// Buscar Mods en Modrinth usando su API pública
async function searchMods() {
    ...
    try {
        // Buscamos proyectos filtrando por versión de minecraft y loader
...
// Mostrar resultados en la columna central
function displayResults(mods) {
    ...
    mods.forEach(mod => {
        ...
        item.className = 'bg-[#161925] p-3 rounded-xl border border-gray-800 flex justify-between items-center gap-2 hover:border-purple-900 transition-colors';
        item.innerHTML = `
            ...
            <button onclick="addMod('${mod.project_id}', '${mod.title}', '${mod.icon_url}')" class="bg-purple-600 hover:bg-purple-700 text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors">
                + Añadir
            </button>
        `;
...
// Añadir mod a la lista de selección
async function addMod(projectId, title, iconUrl) {
    ...
    // Buscamos la versión específica del mod para nuestro Minecraft/Loader
    try {
        ...
        // Agarramos la versión más reciente compatible
...
// Actualizar lista de añadidos visualmente
...
// GENERAR EL ARCHIVO .MRPACK AL INSTANTE
btnExport.addEventListener('click', async () => {
    ...
    // Estructura del index.json de Modrinth
    const indexJson = {
        formatVersion: 1,
        game: "minecraft",
        versionId: packVer,
        name: packName,
        summary: "Modpack creado de forma ultra ligera desde la Web con PackCraft",
    ...
    indexJson.dependencies[loader] = "recommended"; // Requerir loader dinámico

    // Agregar el JSON de configuración al archivo ZIP virtual
    zip.file("modrinth.index.json", JSON.stringify(indexJson, null, 2));
    zip.folder("overrides"); // Carpeta para recursos/configs custom si se desea

    // Compilar el ZIP y forzar la descarga en formato .mrpack
